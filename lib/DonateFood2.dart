...
class DonateFood2 extends StatefulWidget {
  final FoodItem foodItem;

  const DonateFood2({super.key, required this.foodItem});

  @override
  State<DonateFood2> createState() => _DonateFood2State();
}

final db = FirebaseFirestore.instance;
final fooditem = <String, dynamic> {};

Future<bool> donateFoodItem(String type, String quantity, String expiration, String address, String notes) async {
  fooditem["Type"] = type;
  ...
  try{
    db.collection("Donated Food").add(fooditem);
    return true;
  } catch (e) {
    print(e);
    return false;
...
class _DonateFood2State extends State<DonateFood2> {
  ...
  Future<void> _donateFood() async {
    if(_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if(mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Donation sent successfully')),
          );
